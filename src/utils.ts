...
import { DataInit, Metadata } from './metaplex/classes';
...
export function loadMetadata(file?: string): DataInit {
    if(!file) {
        file = path.resolve(__dirname, 'data', 'current-metadata-cache.json');
    }
    return JSON.parse(fs.readFileSync(file).toString());
}

export function readPrivateKey(file?: string): string {
    if(!file) {
